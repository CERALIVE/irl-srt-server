...
int CSLSListener::handler()
{
    ...
    // Dispatch authorization BEFORE the publisher is reachable through the
    // publisher map: a dispatch that fails leaves nothing to authorize, so the
    // role must be torn down here rather than published and then policed.
