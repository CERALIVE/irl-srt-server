...
# --- poll /stats until the publisher has both a parsed audio track and a player ---
stats=""
ready=0
i=0
while [ "$i" -lt 60 ]; do
    ...
    if [ -n "$stats" ] && printf '%s' "$stats" | jq -e \
        '((.publishers // {}) | length) > 0 and (([.publishers[]?.audioGapFill.audioTrackCount] | add) // 0) > 0 and ((.publishers | to_entries[0].value.players // []) | length) > 0' \
        >/dev/null 2>&1; then
        ready=1; break
    fi
...
[ "$ready" -eq 1 ] || { echo "--- last /stats ---" >&2; printf '%s\n' "$stats" >&2; fail "no live publisher with an audio track and player within timeout"; }
