...
TEST_CASE("sls_validate_sid_format: query syntax in a component is rejected")
{
    // Query syntax is not part of the stream-name contract. Reject it at the
    // handshake boundary so every downstream URL builder receives the same
    // conservative identifier alphabet.
    CHECK_FALSE(sls_validate_sid_format("example.com/live/feed1?token=abc"));
}
